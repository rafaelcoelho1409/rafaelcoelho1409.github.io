---
layout: page
title: Skills
nav: true
nav_order: 5
---

<style>
.skill-container {
  display: flex;
  flex-wrap: wrap;
  gap: 30px;
  margin-bottom: 40px;
  justify-content: flex-start;
}

.skill-item {
  width: calc(20% - 24px);
  min-width: 150px;
  text-align: center;
  margin-bottom: 20px;
}

.skill-item a {
  text-decoration: none;
  display: flex;
  flex-direction: column;
  align-items: center;
}

.skill-item img {
  width: 100%;
  max-width: 120px;
  height: 120px;
  object-fit: contain;
  margin-bottom: 10px;
  transition: transform 0.3s ease;
}

.skill-item img:hover {
  transform: scale(1.1);
}

.skill-item span {
  font-size: 0.9rem;
  font-weight: 500;
  color: var(--global-text-color);
}

@media (max-width: 1200px) {
  .skill-item {
    width: calc(25% - 22.5px);
  }
}

@media (max-width: 768px) {
  .skill-item {
    width: calc(33.333% - 20px);
  }
}

@media (max-width: 480px) {
  .skill-item {
    width: calc(50% - 15px);
  }
}
</style>

## Programming Languages

<div class="skill-container">
  <div class="skill-item">
    <a href="https://docs.python.org/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" alt="Python">
      <span>Python</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://www.gnu.org/software/bash/manual/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bash/bash-original.svg" alt="Bash">
      <span>Bash</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://go.dev/doc/" target="_blank">
      <img src="https://raw.githubusercontent.com/devicons/devicon/54cfe13ac10eaa1ef817a343ab0a9437eb3c2e08/icons/go/go-original-wordmark.svg" alt="Go">
      <span>Go</span>
    </a>
  </div>
</div>

## Data Science & Analytics

<div class="skill-container">
  <div class="skill-item">
    <a href="https://numpy.org/doc/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" alt="NumPy">
      <span>NumPy</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://pandas.pydata.org/docs/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" alt="Pandas">
      <span>Pandas</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://pola-rs.github.io/polars/" target="_blank">
      <img src="https://raw.githubusercontent.com/pola-rs/polars-static/master/logos/polars-logo-dark.svg" alt="Polars">
      <span>Polars</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://plotly.com/python/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/plotly/plotly-original.svg" alt="Plotly">
      <span>Plotly</span>
    </a>
  </div>
</div>

## Machine Learning

<div class="skill-container">
  <div class="skill-item">
    <a href="https://scikit-learn.org/stable/documentation.html" target="_blank">
      <img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" alt="Scikit-Learn">
      <span>Scikit-Learn</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://xgboost.readthedocs.io/" target="_blank">
      <img src="https://raw.githubusercontent.com/dmlc/dmlc.github.io/master/img/logo-m/xgboost.png" alt="XGBoost">
      <span>XGBoost</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://www.scikit-yb.org/" target="_blank">
      <img src="https://img.stackshare.io/service/12133/yellobrick.png" alt="YellowBrick">
      <span>YellowBrick</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://umap-learn.readthedocs.io/" target="_blank">
      <img src="https://umap-learn.readthedocs.io/en/latest/_static/logo.png" alt="UMAP">
      <span>UMAP (Uniform Manifold Approximation and Projection)</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://adtk.readthedocs.io/" target="_blank">
      <img src="https://adtk.readthedocs.io/en/stable/_static/arundo_logo_black.png" alt="ADTK">
      <span>ADTK (Anomaly Detection Tool Kit)</span>
    </a>
  </div>
</div>

## Deep Learning

<div class="skill-container">
  <div class="skill-item">
    <a href="https://www.tensorflow.org/api_docs" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" alt="TensorFlow">
      <span>TensorFlow</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://pytorch.org/docs/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" alt="PyTorch">
      <span>PyTorch</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://keras.io/api/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/keras/keras-original.svg" alt="Keras">
      <span>Keras</span>
    </a>
  </div>
</div>

## Generative AI & Large Language Models

<div class="skill-container">
  <div class="skill-item">
    <a href="https://platform.openai.com/docs/" target="_blank">
      <img src="https://www.svgrepo.com/show/306500/openai.svg" alt="OpenAI">
      <span>OpenAI</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://huggingface.co/docs" target="_blank">
      <img src="https://huggingface.co/front/assets/huggingface_logo-noborder.svg" alt="HuggingFace">
      <span>HuggingFace</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://ollama.com/docs" target="_blank">
      <img src="https://ollama.com/public/ollama.png" alt="Ollama">
      <span>Ollama</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://console.groq.com/docs" target="_blank">
      <img src="https://console.groq.com/groq-logo.svg" alt="Groq">
      <span>Groq</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://python.langchain.com/docs/" target="_blank">
      <img src="https://registry.npmmirror.com/@lobehub/icons-static-png/1.74.0/files/dark/langchain-color.png" alt="LangChain">
      <span>LangChain</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://langchain-ai.github.io/langgraph/" target="_blank">
      <img src="https://registry.npmmirror.com/@lobehub/icons-static-png/1.74.0/files/dark/langgraph.png" alt="LangGraph">
      <span>LangGraph</span>
    </a>
  </div>
</div>

## Computer Vision

<div class="skill-container">
  <div class="skill-item">
    <a href="https://docs.opencv.org/4.x/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/opencv/opencv-original.svg" alt="OpenCV">
      <span>OpenCV</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.ultralytics.com/" target="_blank">
      <img src="https://raw.githubusercontent.com/ultralytics/assets/main/logo/Ultralytics_Logotype_Original.svg" alt="Ultralytics">
      <span>Ultralytics</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://developers.google.com/mediapipe" target="_blank">
      <img src="https://viz.mediapipe.dev/logo.png">
      <span>MediaPipe</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://roboflow.com/docs" target="_blank">
      <img src="https://logosandtypes.com/wp-content/uploads/2025/02/roboflow.png" alt="RoboFlow">
      <span>RoboFlow</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.openvino.ai/" target="_blank">
      <img src="https://images.seeklogo.com/logo-png/46/1/openvino-logo-png_seeklogo-468085.png" alt="OpenVINO">
      <span>OpenVINO</span>
    </a>
  </div>
</div>

## MLOps & DevOps

<div class="skill-container">
  <div class="skill-item">
    <a href="https://git-scm.com/doc" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" alt="Git">
      <span>Git</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.github.com/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" alt="GitHub">
      <span>GitHub</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.gitlab.com" target="_blank">
      <img src="https://raw.githubusercontent.com/devicons/devicon/54cfe13ac10eaa1ef817a343ab0a9437eb3c2e08/icons/gitlab/gitlab-original.svg" alt="GitLab">
      <span>GitLab</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.docker.com/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" alt="Docker">
      <span>Docker</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://kubernetes.io/docs/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" alt="Kubernetes">
      <span>Kubernetes</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://helm.sh/docs/" target="_blank">
      <img src="https://raw.githubusercontent.com/devicons/devicon/54cfe13ac10eaa1ef817a343ab0a9437eb3c2e08/icons/helm/helm-original.svg" alt="Helm">
      <span>Helm</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://developer.hashicorp.com/terraform/docs" target="_blank">
      <img src="https://raw.githubusercontent.com/devicons/devicon/54cfe13ac10eaa1ef817a343ab0a9437eb3c2e08/icons/terraform/terraform-original.svg" alt="Terraform">
      <span>Terraform</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://argo-cd.readthedocs.io/en/stable/" target="_blank">
      <img src="https://raw.githubusercontent.com/devicons/devicon/54cfe13ac10eaa1ef817a343ab0a9437eb3c2e08/icons/argocd/argocd-original.svg" alt="ArgoCD">
      <span>ArgoCD</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://k3d.io/stable/" target="_blank">
      <img src="https://k3d.io/stable/static/img/k3d_logo_black_blue.svg" alt="K3D">
      <span>K3D</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://skaffold.dev/docs/" target="_blank">
      <img src="https://raw.githubusercontent.com/GoogleContainerTools/skaffold/main/logo/skaffold.png" alt="Skaffold">
      <span>Skaffold</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://mlflow.org/docs/latest/index.html" target="_blank">
      <img src="https://raw.githubusercontent.com/mlflow/mlflow/refs/heads/master/assets/logo.svg" alt="MLflow">
      <span>MLflow</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://fastapi.tiangolo.com/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" alt="FastAPI">
      <span>FastAPI</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.pydantic.dev/" target="_blank">
      <img src="https://docs.pydantic.dev/latest/logo-white.svg" alt="Pydantic">
      <span>Pydantic</span>
    </a>
  </div>
</div>

## Data Engineering

<div class="skill-container">
  <div class="skill-item">
    <a href="https://spark.apache.org/docs/latest/api/python/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachespark/apachespark-original.svg" alt="PySpark">
      <span>PySpark</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.delta.io/" target="_blank">
      <img src="https://delta.io/_astro/delta-lake-logo.Dx7tzbyv_1OsUys.svg" alt="Delta Lake">
      <span>Delta Lake</span>
    </a>
  </div>
</div>

## Web Development & UI Frameworks

<div class="skill-container">
  <div class="skill-item">
    <a href="https://docs.streamlit.io/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/streamlit/streamlit-original.svg" alt="Streamlit">
      <span>Streamlit</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://dash.plotly.com/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/plotly/plotly-original.svg" alt="Plotly Dash">
      <span>Plotly Dash</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://doc.qt.io/qtforpython-6/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/qt/qt-original.svg" alt="PyQT6">
      <span>PyQT6</span>
    </a>
  </div>
</div>

## Automation & Orchestration

<div class="skill-container">
  <div class="skill-item">
    <a href="https://www.selenium.dev/documentation/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/selenium/selenium-original.svg" alt="Selenium">
      <span>Selenium</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://pyautogui.readthedocs.io/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" alt="PyAutoGUI">
      <span>PyAutoGUI</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://documentation.botcity.dev/" target="_blank">
      <img src="https://cdn.prod.website-files.com/6487203eebdb29c0fff6af27/66193a64d8ee208d28e3796f_logo_botcity_2024.svg" alt="BotCity">
      <span>BotCity</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://airflow.apache.org/docs/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apacheairflow/apacheairflow-original.svg" alt="Apache Airflow">
      <span>Apache Airflow</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://luigi.readthedocs.io/" target="_blank">
      <img src="https://raw.githubusercontent.com/spotify/luigi/master/doc/luigi.png" alt="Luigi">
      <span>Luigi</span>
    </a>
  </div>
</div>

## Cloud Platforms

<div class="skill-container">
  <div class="skill-item">
    <a href="https://docs.aws.amazon.com/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" alt="AWS">
      <span>AWS</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://learn.microsoft.com/azure/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" alt="Azure">
      <span>Azure</span>
    </a>
  </div>
</div>

## Databases

<div class="skill-container">
  <div class="skill-item">
    <a href="https://dev.mysql.com/doc/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" alt="MySQL">
      <span>MySQL</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://www.postgresql.org/docs/" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" alt="PostgreSQL">
      <span>PostgreSQL</span>
    </a>
  </div>
</div>

## Cybersecurity

<div class="skill-container">
  <div class="skill-item">
    <a href="https://docs.metasploit.com/" target="_blank">
      <img src="https://assets.streamlinehq.com/image/private/w_300,h_300,ar_1/f_auto/v1/icons/logos/metasploit-h33wivgvlvgtk3wmoejxq.png/metasploit-rkuireddk3p5vdgf5mil5.png" alt="Metasploit">
      <span>Metasploit</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://portswigger.net/burp/documentation" target="_blank">
      <img src="https://images.icon-icons.com/3053/PNG/512/burp_suite_macos_bigsur_icon_190319.png">
      <span>Burp Suite</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://nmap.org/book/man.html" target="_blank">
      <img src="https://nmap.org/images/nmap-logo-256x256.png" alt="NMAP">
      <span>NMAP</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://github.com/sullo/nikto/wiki" target="_blank">
      <img src="https://cirt.net/wp-content/uploads/2025/08/alienlogo_vectorized_500x609-768x936.png" alt="Nikto">
      <span>Nikto</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://docs.projectdiscovery.io/" target="_blank">
      <img src="https://projectdiscovery.io/_next/static/media/ProjectDiscoveryLogo.a524f50f.svg" alt="ProjectDiscovery">
      <span>ProjectDiscovery</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://developer.shodan.io/" target="_blank">
      <img src="https://www.shodan.io/static/img/logo-6abcc86b.png" alt="Shodan">
      <span>Shodan</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://search.censys.io/api" target="_blank">
      <img src="https://search.censys.io/static/img/censys-2025.svg" alt="Censys">
      <span>Censys</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://www.qemu.org/docs/master/" target="_blank">
      <img src="https://images.icon-icons.com/2699/PNG/512/qemu_logo_icon_169821.png" alt="QEMU">
      <span>QEMU</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://www.kali.org/docs/" target="_blank">
      <img src="https://www.kali.org/images/kali-logo.svg" alt="Kali Linux">
      <span>Kali Linux</span>
    </a>
  </div>
  <div class="skill-item">
    <a href="https://ubuntu.com/server/docs" target="_blank">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ubuntu/ubuntu-plain.svg" alt="Ubuntu">
      <span>Ubuntu</span>
    </a>
  </div>
</div>
